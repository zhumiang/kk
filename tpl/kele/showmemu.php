...
<table width="90%" border="0" cellpadding="0" cellspacing="1" bgcolor="#C9C9C9" style="margin:10px;">
	  ...
	  if ($page=keledata::httpval("page"))keledata::checkdata($page,"number");
		!$page && $page=1;$size=20;
		$sum=keledatabase::getnum("kele_memu");
		keledata::pagemover($size,$sum,"?model=show&contro=function&view=memus",$page);
		$limit=array("start"=>($page-1)*$size,"num"=>$size);
	  $data=keledatabase::getarray("kele_memu","","*","id desc","",$limit);
	  foreach ($data as $value){
	  ?>
	  	<td><?php echo $value['id']?></td>
        <td><?php echo $value['name']?></td>
        <td><?php echo $value['value']?></td>
        <td><a href="kele.php?model=amend&contro=function&view=memus&id=<?php echo $value['id']?>">Form Manage</a>
        &nbsp;<a href="kele.php?model=delete&contro=function&view=memus&id=<?php echo $value['id']?>" onClick="return confirm('Are you sure you want to delete?');" >Delete</a></td>
      </tr>
      <?php }?>
