...
<a href="kele.php?model=show&contro=function&view=memus">Read All</a>
</div>
	<?php
	$memu=kelefunction::memu($this->view);
	if($this->model=="amend"){
		$action="kele.php?model=amend&contro=function&view=memus";
	}else {
		...
		$data_memu=keledatabase::getthisone("kele_memu","value='".$lord['value']."'");
		if(count($data_memu)>0){
			$data_memufield=keledatabase::getarray("kele_memufield","memuid='".$data_memu['id']."'");			
			foreach ($data_memufield as $memufieldval){
				$data_memufield[$memufieldval['field']]=$memufieldval;
			}		
		}
		$action="kele.php?model=append&contro=function&view=memus";
	}?>
	<form name="<?php echo $data['memu']['value']?>" action="<?php echo $action?>" method="post">
      ...
      <tr>
      	<?php foreach($memu['element']['memus'] as $k => $val){
      	...
      	<input type="hidden" name="memus[id]" value="<?php echo $GLOBALS['DATA']['memus']['id']?>">
    </table>
    ...
    <table width="90%" border="0" cellpadding="0" cellspacing="1" bgcolor="#C9C9C9" style="margin:10px;">
      	<?php if($this->model!="amend"){$i=0;
      		...
      		foreach($memu['element']['memufield'] as $k => $val){
      		...
      		$i++;}}
      	?>
      	<?php }else{foreach ($GLOBALS['DATA']['memufield'] as $key=>$value){
      	...
      	<input type="hidden" name="memufield[<?php echo $value['id']?>][id]" value="<?php echo $value['id']?>">
      	<?php }}?>
      </table>
		<div align="center">
