...
<?php
	$wid=keledata::httpval("wid")?keledata::httpval("wid"):$GLOBALS['DATA']['workreport']['weeklyplanid'];
	if(keledata::checkdata($wid,"int"))$pname=keledatabase::getthisone(array("weekly_plan","project_management"),
			array("weekly_plan.id=".$wid,"weekly_plan.projectid=project_management.id"),
...
?>
    ...
    <table width="90%" border="0" cellpadding="0" cellspacing="1" bgcolor="#C9C9C9" style="margin:10px;">
      	...
      	foreach($memus as $k => $val){
      		...
      		else if($val['field']=="weeklyplanid"&&$wid)$GLOBALS['DATA'][$val['classify']][$val['field']]=$wid;      		
