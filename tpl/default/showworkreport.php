...
<div class="menu"> 
<?php 
if($GLOBALS['POWER'][$this->view]['append']){
$wid=keledatabase::getthisone("weekly_plan","plantime='".date("Y-m-d",time())."'","id");
if($wid){
	?>
<a href="<?php echo http_dir?>?model=append&contro=function&view=<?php echo $this->view?>&wid=<?php echo $wid?>">AddFrom</a>&nbsp;
<?php }}?>
...
<?php if(keledata::httpval('id')){
	...
	$pname=keledatabase::getthisone(array("weekly_plan","project_management"),
			array("weekly_plan.id=".$GLOBALS['DATA']['weeklyplanid'],"weekly_plan.projectid=project_management.id"),
