...
class indexmember {
	...
	public function login($view){
		...
		if($data['login']==true){
			...
			if(keledata::checkdata($data['username'],'',false)||keledata::checkdata($data['username'],'chinese','',false) 
					&& keledata::checkdata($data['password'],'',false)){
				$this->userdb=keledatabase::getthisone("users","username='".$data['username']."'");
				if($this->userdb){
					if($this->userdb['password']==md5($data['password'])){
						if($this->userdb['ispass']!="true")kele::exception('error','unauthor');
						indexmember::onlineuser($view);
	...
	public function onlineuser($view){
		...
		$this->online=keledatabase::getthisone("kele_useron",array("userid='".$this->userdb['id']."'",
				"username='".$this->userdb['username']."'"));
		if($this->online['userid']&&$this->online['onlineip']!=$this->ip && (time()-$this->online['onlinetime']<300)){
			keledata::delsession(array("userid","username","onlineip","departmentid","positionid","usergroupid"));
			kele::exception('error','havelogin');
		}else{
			if(!$userdb['userid']){
				keledatabase::delete("kele_useron","userid='".$this->online['userid']."'");
				$groupid = keledatabase::getthisone(array("users","position"),array("users.id=".$this->userdb['id'],
						"position.id=users.positionid"),"position.usergroupid");
				keledata::setsession(array('userid'=>$this->userdb['id'],'username'=>$this->userdb['username'],
				'onlineip'=>$this->ip,'departmentid'=>$this->userdb['departmentid'],
				'positionid'=>$this->userdb['positionid'],'usergroupid'=>$groupid));
				keledatabase::insert(array($this->userdb['id'],$this->userdb['username'],
				$this->ip,time()),'kele_useron',array('userid','username','onlineip','onlinetime'));
			}elseif($this->online['userid'] && $userdb['userid']){
				keledatabase::update("kele_useron","onlinetime='".time()."'",array("userid='".$userdb['userid']."'",
				"username='".$userdb['username']."'","onlineip='".$userdb['onlineip']."'"));
			}else {
				keledata::delsession(array("userid","username","onlineip","departmentid","positionid","usergroupid"));
